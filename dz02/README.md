...
```
можно посмотреть какое устройтво подключено к тому или иному порту или воспользоваться сервисами, которые определяют вендора и устройство по mac-адресу
```
...
```
В крупных сетях хостам необходимы большие ARP-таблицы для обработки сопоставления IP-адресов и MAC-адресов. Это снижает пропускную способность сети и является проблемой безопасности, т.к. один компьютер может выдавать себя за другой для перехвата пакетов. Это называется "спуфингом".
```
